...
def is_short_message(body: str) -> bool:
    ...
    short_responses = {
        'ok', 'k', 'kk', 'yes', 'no', 'yeah', 'nah', 
        'sure', 'thanks', 'thx', 'ty', 'np', 'lol'
    }
    
    # Check if it's a short response or very short message
    return (text in short_responses or
            len(text.split()) <= 2 or  # 2 or fewer words
            len(text) <= 5)            # 5 or fewer characters

def process_messages(conn: sqlite3.Connection, xml_path: Path, last_run_date: datetime = None) -> int:
    """
